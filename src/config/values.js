...
export const values = {
  ...
  combat: {
    ...
    // 战斗力系数阈值（血量口径，capability.js）：血量低于它的比例后，能力随血量线性下降。
    // **伤害与交战补给消耗共用这一条曲线**——改这个值，两边一起变（gdd.md §4、§6）。
    hp_dps_ratio: 0.8,
  ...
  supplyStock: {
    ...
    perSecond: {
      ...
      inCombat: -4, // 交战中（正被敌方瞄准）的补给消耗。⚠️ 与下面的 inCombatSupport 都要
                    // × 战斗力系数（capability.js）：同一状态下伤得越重、交战消耗越少
      // 参战但当前**没有**被敌方瞄准（state=combat 且 !underFire，例如两个单位打同一个敌人时
      // 只有前排被还击）：同样消耗补给，但比面对面的单位少（同样乘战斗力系数）
      inCombatSupport: -3,
      moving: -2.5,   // 行军消耗
