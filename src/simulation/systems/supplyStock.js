...
function consume(world, unit, dt) {
  ...
  if (unit.underFire) {
    rate += cfg.perSecond.inCombat * mode; // 正被敌方瞄准：全额交战消耗
  } else if (unit.state === 'combat') {
    // 参战但当前没被瞄准（例如两个单位打同一个敌人时，只有前排被还击）：消耗少一些
    rate += cfg.perSecond.inCombatSupport * mode;
  }
...
function routRecovery(unit, dt) {
  const cfg = values.supplyStock;
  const combatPenalty = unit.underFire ? cfg.perSecond.inCombat : 0; // 本身已是负数
