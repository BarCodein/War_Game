...
| 附近己方城市（≤120 px） | +5 /s |
| 补给充足（实收 ≥ 需求） | +1 /s |
| 缺补给（实收 < 需求） | −2 /s × 缺口比例（实收 60% → 只吃 40% 的惩罚） |
...
- 每座己方城市有 **5 点**吞吐（`capacityPerCity`），每个单位需求 **1 点**（`demandPerUnit`）。
...
- 城市要把补给送到单位手里，需要付出 **需求 ÷ 因子** 点（远距离运输有损耗），单位实收 = 付出点数 × 因子。
- 因此**近城一座就能喂饱一个单位，远城要好几座城合力**：一座城的剩余运力不够时先给一部分，差额由下一座城补齐（`unit.supplyEdges` 会记录多条边）。

**4. 缺补给的后果（按缺口比例）**

- 实收 ≥ 需求 → 满补给（`supplied = true`）：士气 +1 /s。
- 实收 < 需求 → 缺补给：生命 **−1 /s × 缺口比例**、士气 **−2 /s × 缺口比例**。
- 实收 = 0 → 完全断补：生命 −1 /s、士气 −2 /s（损耗同样计入结算伤亡，`cause = attrition`）。
...
> 恢复（§7）仍按直线距离判定：己方城市 100 px 内 +3 血/s。生产暂停条件改为「该城本轮运力支出 ≥ 容量」。
...
- **实际控制线**（视觉表现，非需求项）：把地图切成 **20 px 方网格**，每格累加双方**影响力**——
  影响力源 = 存活单位（半径 140 / 满强度 100）+ 城市（半径 140 / 强度 80）+ 占领点（半径 180 / 强度 60），
...
- **顶栏**：暂停/继续、声音开关（保留）。
- **左栏**：编队列表（血量、士气、状态、**补给百分比**）；城市状态卡（占领进度、**已补给单位 n/m**、**城市运力 x/y 点**；生产已关闭，故不显示生产倒计时）。
...
| supply：demandPerUnit / capacityPerCity | 1 点需求 / 5 点吞吐（每座己方城市） |
| supply：attritionHpPerSecond / attritionMoralePerSecond | −1 /s / −2 /s（**完全断补**时的值；实收不足时按缺口比例缩放） |
...
| controlLine.capturePoint：influenceRadius / strength / coreRadius | 180 / 60 / **20**（同样与占领半径 60 脱钩：核心圈覆盖标记本体并守住据点脚下那一格；< 14 时单个敌军贴上来就能翻掉那一格） |
