...
- [ ] A1 轻型与重型单位属性（生命/伤害/间隔/距离/速度/视野）来自配置且符合 gdd.md 数值表 — Vitest
...
- [ ] B1 己方城市每 12 s 自动生产 1 个轻型单位，补给容量满时暂停 — Vitest
- [ ] B2 每座己方城市默认补给容量 5 — Vitest
- [ ] B3 超出补给上限的单位生命受损耗且士气下降（−1 /s、−2 /s）— Vitest
- [ ] B4 占领进度：每单位 5% /s、上限 15% /s；敌方单位在场冻结；离开后 3% /s 衰减 — Vitest
- [ ] B5 占领进度 100% 时城市变更阵营 — Vitest
- [ ] B6 己方城市附近恢复生命与士气（+3 /s、+5 /s）— Vitest

## C. 战斗、地形与视野

- [ ] C1 敌对单位进入交战距离自动攻击 — Vitest
- [ ] C2 伤害公式含地形修正（森林 0.85、桥梁 0.9）— Vitest
- [ ] C3 四种地形通行性与移动倍率正确（水域仅桥梁可通行）— Vitest
- [ ] C4 战争迷雾正确区分三态：当前可见 / 已探索 / 从未探索 — Vitest + Playwright
- [ ] C5 不可见敌军不显示实时状态；显示最后已知位置虚影并在重新目视后更新 — Vitest + Playwright
- [ ] C6 森林视野阻挡：森林中敌军仅在 60 px 内可见 — Vitest

## D. 士气

- [ ] D1 士气受友军/城市/补给/交战/友军阵亡影响，修正值符合 gdd.md §6 — Vitest
- [ ] D2 低士气削弱效果（削弱 ×0.75 伤害，动摇 ×0.5）— Vitest
- [ ] D3 士气归零触发溃逃（不受指挥、向己方城市撤退、恢复至 20 停止）— Vitest
- [ ] D4 无法撤退（被困或无己方城市）时投降移除 — Vitest
- [ ] D5 士气变化有 UI 反馈（血条旁士气条变色、状态标签）— Playwright

## E. 关卡、控制器与胜负

- [ ] E1 教学关卡完整闭环可玩：选择 → 指挥 → 交战 → 观察迷雾与士气 → 占领 → 胜负结算 — Playwright
- [ ] E2 脚本敌军支持驻守、定时增援、条件触发进攻、预设目标移动 — Vitest
- [ ] E3 脚本敌军经统一命令接口控制世界（与玩家输入同一通道）— 评审 + Vitest
- [ ] E4 胜利条件：占领全部敌方城市；或消灭全部敌军且敌方无可生产城市 — Vitest
- [ ] E5 失败条件与胜利对称 — Vitest
...
- [ ] F10 地图为版本化 JSON，读取时结构校验并具备迁移机制 — Vitest

## G. 数据、表现与性能

- [ ] G1 固定时间步长模拟与帧率无关（同输入下结果确定）— Vitest
- [ ] G2 无全单位两两检测（空间分区邻居查询）— 评审 + Vitest
