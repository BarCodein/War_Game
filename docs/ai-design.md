...
2. **`weights.interdiction` 目前实际是个开关**：0.1 / 0.2 / 0.35 / 0.6 的 44 项指标逐位完全相同。
   ...
   要把它变成真正的旋钮，可以让权重同时缩放断粮任务的活动半径（越看重越愿意绕远）——**本轮未做**，
   留作下一轮的候选（要么承认它是开关，要么改语义）。
...
| — | 断粮权重的语义（开关 vs 旋钮）——见 §5.3 教训 2 | 未做（建议下一轮） |
...
36. 本轮不做：自动写回源码、贝叶斯/进化搜索、把调参结果做成 CI 慢断言、
    修改 `weights.interdiction` 的语义（开关 → 旋钮，见 §5.3 教训 2）。
